...
- <span style={{ color: 'var(--gray50)' }}>[Growth + Enterprise licence tier]</span> Customer-managed KMS (platform-provisioned CMKs per encryption domain)
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> Bring-your-own-key (BYOK) ARNs per encryption domain
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> Optional internal ALB TLS (east-west HTTPS on the internal listener)
...
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> HA endpoint AZ mode (ALB and VPC interface endpoints spanning AZs)
...
- <span style={{ color: 'var(--gray50)' }}>[Growth + Enterprise licence tier]</span> Optional managed observability (AMP remote write + AMG + X-Ray OTLP export)
...
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> Regional CloudTrail (deployment region plus global IAM/STS) with log-file integrity validation
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> Protected evidence storage (versioning, lifecycle, S3 Object Lock COMPLIANCE, stage-aware retention)
- <span style={{ color: 'var(--gray50)' }}>[Enterprise-only licence tier]</span> ALB access logs into a companion evidence bucket
