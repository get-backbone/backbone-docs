...
To allow consumption of GitHub packages published by the public `get-backbone/backbone-kit` and `get-backbone/backbone-community` repos, configure local `~/.m2/settings.xml` with GitHub credentials. The script prompts for your GitHub username and a classic (only) PAT token named `PAT_BACKBONE_DEPLOY` that you will need to create. The script will back up any existing `~/.m2/settings.xml` file first.
